...
walkFolder(helpTopicsFolder).forEach((filePath) => {
  const helpTopicDir = path.dirname(filePath);
  const helpTopicName = path.basename(filePath, path.extname(filePath)).toLowerCase();
  ...
  const formattedLines = helpTopic
    .replaceAll(/#include[ ]+"([^"]+)"[ ]*\n/g, (match, includeFileName, offset, src) => {
      const includePath = path.resolve(helpTopicDir, includeFileName);
    ...
    .map((line) => line.replaceAll(/!!!!!!/g, '%'))
    .map((line) => line.replaceAll(/@@TOCHAR_([0-9a-fA-F]{4})@@/g, "' + (TOCHAR 0x$1) + '"))
